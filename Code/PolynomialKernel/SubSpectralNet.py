...
while(overlap*i <= melSize - splitSize):
    ...
    CONV = KernelConv2D(filters=32, 
                        kernel_size=7, 
                        padding='same',
                        kernel_function=PolynomialKernel(p=2, trainable_c=True))(INPUT)
    ...
    CONV = KernelConv2D(filters=64, 
                        kernel_size=7, 
                        padding='same',
                        kernel_function=PolynomialKernel(p=2, trainable_c=True))(CONV)
    CONV = BatchNormalization(axis=1,
                             gamma_regularizer=l2(0.0001),
                             beta_regularizer=l2(0.0001))(CONV)
    CONV = Activation('relu')(CONV)
...
type = 'SubSpectralNet-Polynomial-' + str(melSize) + '_' + str(splitSize) + '_' + str(overlap)
